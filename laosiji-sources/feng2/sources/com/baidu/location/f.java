...
public class f extends Service {
    ...
    private boolean readConf(File file) {
        boolean z = false;
        try {
            File file2 = new File(k.h() + "/grtcfrsa.dat");
            if (file2.exists()) {
                ...
                if (randomAccessFile.readBoolean() && randomAccessFile.readBoolean()) {
                    ...
                    if (readInt != 0) {
                        ...
                        if (!(str == null || a == null || !k.b(a, str, "MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQCiP7BS5IjEOzrKGR9/Ww9oSDhdX1ir26VOsYjT1T6tk2XumRpkHRwZbrucDcNnvSB4QsqiEJnvTSRi7YMbh2H9sLMkcvHlMV5jAErNvnuskWfcvf7T2mq7EUZI/Hf4oVZhHV0hQJRFVdTcjWI6q2uaaKM3VMh+roDesiE7CR2biQIDAQAB"))) {
                            z = true;
                        }
                    }
                }
    ...
    public void onCreate() {
        mC = StubApp.getOrigApplicationContext(getApplicationContext());
        System.currentTimeMillis();
        this.libNat = new a();
        try {
            File file = new File(k.h() + File.separator + replaceFileName);
            File file2 = new File(k.h() + File.separator + jarFileName);
            if (file.exists()) {
                if (file2.exists()) {
                    file2.delete();
                }
            ...
            if (file2.exists() && readConf(new File(k.h() + File.separator + jarFileName))) {
                this.libJar = (LLSInterface) new DexClassLoader(k.h() + File.separator + jarFileName, k.h(), null, getClassLoader()).loadClass("com.baidu.serverLoc.LocationService").newInstance();
            }
        } catch (Exception e) {
            this.libJar = null;
        }
