...
public class RequestLoggingListener implements RequestListener {
    ...
    public synchronized void onRequestStart(ImageRequest request, Object callerContextObject, String requestId, boolean isPrefetch) {
        if (FLog.isLoggable(2)) {
            FLog.v(TAG, "time %d: onRequestSubmit: {requestId: %s, callerContext: %s, isPrefetch: %b}", Long.valueOf(getTime()), (Object) requestId, callerContextObject, Boolean.valueOf(isPrefetch));
    ...
    public synchronized void onProducerStart(String requestId, String producerName) {
        if (FLog.isLoggable(2)) {
            ...
            FLog.v(TAG, "time %d: onProducerStart: {requestId: %s, producer: %s}", Long.valueOf(startTime), (Object) requestId, (Object) producerName);
        }
    }
    ...
    public synchronized void onRequestSuccess(ImageRequest request, String requestId, boolean isPrefetch) {
        if (FLog.isLoggable(2)) {
            ...
            long currentTime = getTime();
            FLog.v(TAG, "time %d: onRequestSuccess: {requestId: %s, elapsedTime: %d ms}", Long.valueOf(currentTime), (Object) requestId, Long.valueOf(getElapsedTime(startTime, currentTime)));
        }
    }
    ...
    public synchronized void onRequestCancellation(String requestId) {
        if (FLog.isLoggable(2)) {
            Long startTime = (Long) this.mRequestStartTimeMap.remove(requestId);
            long currentTime = getTime();
            FLog.v(TAG, "time %d: onRequestCancellation: {requestId: %s, elapsedTime: %d ms}", Long.valueOf(currentTime), (Object) requestId, Long.valueOf(getElapsedTime(startTime, currentTime)));
        }
    }
