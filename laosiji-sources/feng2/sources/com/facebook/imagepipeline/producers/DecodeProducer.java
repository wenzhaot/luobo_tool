...
public class DecodeProducer implements Producer<CloseableReference<CloseableImage>> {
    ...
    private abstract class ProgressiveDecoder extends DelegatingConsumer<EncodedImage, CloseableReference<CloseableImage>> {
        ...
        private Map<String, String> getExtraMap(@Nullable CloseableImage image, long queueTime, QualityInfo quality, boolean isFinal) {
            ...
            String cacheChoiceStr = String.valueOf(this.mProducerContext.getImageRequest().getCacheChoice());
            if (!(image instanceof CloseableStaticBitmap)) {
                return ImmutableMap.of("queueTime", queueStr, DecodeProducer.HAS_GOOD_QUALITY_KEY, qualityStr, DecodeProducer.IS_FINAL_KEY, finalStr, DecodeProducer.IMAGE_TYPE_KEY, cacheChoiceStr);
            }
            Bitmap bitmap = ((CloseableStaticBitmap) image).getUnderlyingBitmap();
            return ImmutableMap.of(DecodeProducer.BITMAP_SIZE_KEY, bitmap.getWidth() + "x" + bitmap.getHeight(), "queueTime", queueStr, DecodeProducer.HAS_GOOD_QUALITY_KEY, qualityStr, DecodeProducer.IS_FINAL_KEY, finalStr, DecodeProducer.IMAGE_TYPE_KEY, cacheChoiceStr);
        }
     ...
     */
        ...
        private void handleResult(CloseableImage decodedImage, boolean isFinal) {
            CloseableReference decodedImageRef = CloseableReference.of(decodedImage);
            try {
                maybeFinish(isFinal);
