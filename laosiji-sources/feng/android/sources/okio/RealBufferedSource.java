...
final class RealBufferedSource implements BufferedSource {
    ...
    public long read(Buffer sink, long byteCount) throws IOException {
        ...
        } else if (this.buffer.size == 0 && this.source.read(this.buffer, IjkMediaMeta.AV_CH_TOP_FRONT_CENTER) == -1) {
            return -1;
        } else {
    ...
    public boolean request(long byteCount) throws IOException {
        if (byteCount < 0) {
            ...
            while (this.buffer.size < byteCount) {
                if (this.source.read(this.buffer, IjkMediaMeta.AV_CH_TOP_FRONT_CENTER) == -1) {
                    return false;
                }
            }
            return true;
    ...
    public int read(byte[] sink, int offset, int byteCount) throws IOException {
        ...
        if (this.buffer.size == 0 && this.source.read(this.buffer, IjkMediaMeta.AV_CH_TOP_FRONT_CENTER) == -1) {
            return -1;
        }
    ...
    public String readUtf8LineStrict(long limit) throws IOException {
        ...
        if (scanLength < Long.MAX_VALUE && request(scanLength) && this.buffer.getByte(scanLength - 1) == ar.k) {
            if (request(1 + scanLength) && this.buffer.getByte(scanLength) == (byte) 10) {
    ...
    public long indexOf(byte b, long fromIndex, long toIndex) throws IOException {
        if (this.closed) {
            ...
            while (fromIndex < toIndex) {
                ...
                if (lastBufferSize >= toIndex || this.source.read(this.buffer, IjkMediaMeta.AV_CH_TOP_FRONT_CENTER) == -1) {
                    return -1;
                }
