...
public final class Buffer implements BufferedSource, BufferedSink, Cloneable, ByteChannel {
    ...
    public String readUtf8LineStrict(long limit) throws EOFException {
        ...
        if (scanLength < size()) {
            if (getByte(scanLength - 1) == ar.k && getByte(scanLength) == (byte) 10) {
    ...
    public long writeAll(Source source) throws IOException {
        ...
        while (true) {
            long readCount = source.read(this, IjkMediaMeta.AV_CH_TOP_FRONT_CENTER);
            if (readCount == -1) {
            ...
            while (byteCount > 0) {
                if (byteCount < ((long) (source.head.limit - source.head.pos))) {
                    ...
                    if (tail != null && tail.owner) {
                        if ((byteCount + ((long) tail.limit)) - ((long) (tail.shared ? 0 : tail.pos)) <= IjkMediaMeta.AV_CH_TOP_FRONT_CENTER) {
