...
public final class Cookie {
    ...
    private static int dateCharacterOffset(String input, int pos, int limit, boolean invert) {
        for (int i = pos; i < limit; i++) {
            ...
            if ((c >= 32 || c == 9) && c < 127 && ((c < 48 || c > 57) && ((c < 97 || c > IjkMediaMeta.FF_PROFILE_H264_HIGH_422) && ((c < 65 || c > 90) && c != 58)))) {
                dateCharacter = false;
            } else {
                dateCharacter = true;
            }
    ...
    private static String parseDomain(String s) {
        if (s.endsWith(FileUtils.FILE_EXTENSION_SEPARATOR)) {
            throw new IllegalArgumentException();
        }
        if (s.startsWith(FileUtils.FILE_EXTENSION_SEPARATOR)) {
            s = s.substring(1);
        }
    ...
    String toString(boolean forObsoleteRfc2965) {
        ...
        if (this.persistent) {
            if (this.expiresAt == Long.MIN_VALUE) {
                ...
                result.append("; expires=").append(okhttp3.internal.http.HttpDate.format(new Date(this.expiresAt)));
            }
        }
        if (!this.hostOnly) {
            result.append("; domain=");
            if (forObsoleteRfc2965) {
                result.append(FileUtils.FILE_EXTENSION_SEPARATOR);
            }
