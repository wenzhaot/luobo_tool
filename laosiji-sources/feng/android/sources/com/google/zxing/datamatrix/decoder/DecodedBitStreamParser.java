...
final class DecodedBitStreamParser {
    ...
    L_0x0043:
        ...
        return r6;
    L_0x0047:
        r6 = com.google.zxing.datamatrix.decoder.DecodedBitStreamParser.AnonymousClass1.$SwitchMap$com$google$zxing$datamatrix$decoder$DecodedBitStreamParser$Mode;
        r7 = r2.ordinal();
        r6 = r6[r7];
        switch(r6) {
    ...
    private static void decodeBase256Segment(BitSource bits, StringBuilder result, Collection<byte[]> byteSegments) throws FormatException {
        ...
        } else if (d1 < FengConstant.IMAGE_SMALL_WIDTH) {
            count = d1;
            codewordPosition = codewordPosition2;
        } else {
            codewordPosition = codewordPosition2 + 1;
            count = ((d1 - 249) * FengConstant.IMAGE_SMALL_WIDTH) + unrandomize255State(bits.readBits(8), codewordPosition2);
        }
        if (count < 0) {
            throw FormatException.getFormatInstance();
        }
    ...
    private static int unrandomize255State(int randomizedBase256Codeword, int base256CodewordPosition) {
        ...
        return tempVariable >= 0 ? tempVariable : tempVariable + AbstractMessageHandler.MESSAGE_TYPE_PUSH_SWITCH_STATUS;
    }
}
