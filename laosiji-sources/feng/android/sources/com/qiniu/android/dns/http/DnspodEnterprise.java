...
public final class DnspodEnterprise implements IResolver {
    ...
    public Record[] resolve(Domain domain, NetworkInfo info) throws IOException {
        HttpURLConnection httpConn = (HttpURLConnection) new URL("http://" + this.ip + "/d?ttl=1&dn=" + encrypt(domain.domain) + "&id=" + this.id).openConnection();
        ...
        String[] r1 = decrypt(new String(data, 0, read)).split(MiPushClient.ACCEPT_TIME_SEPARATOR);
        if (r1.length != 2) {
            return null;
        }
        try {
    ...
    private String encrypt(String domain) {
        try {
            ...
            return Hex.encodeHexString(cipher.doFinal(domain.getBytes(Constants.UTF_8))) + "&id=" + this.id;
        } catch (Exception e) {
            e.printStackTrace();
            return "";
        }
