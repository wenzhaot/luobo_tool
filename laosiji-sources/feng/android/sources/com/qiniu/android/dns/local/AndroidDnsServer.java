    ...
    public static IResolver defaultResolver() {
        return new IResolver() {
            public Record[] resolve(Domain domain, NetworkInfo info) throws IOException {
                ...
                int length;
                Record r;
                Record[] records = new HijackingDetectWrapper(new Resolver(addresses[0])).resolve(domain, info);
                if (domain.hasCname) {
                    boolean cname = false;
                ...
                if (domain.maxTtl != 0) {
                    ...
                    while (i < length) {
                        ...
                        if (r2.isCname() || r2.ttl <= domain.maxTtl) {
                            ...
                            throw new DnshijackingException(domain.domain, addresses[0].getHostAddress(), r2.ttl);
                        }
                    }
                }
                return records;
