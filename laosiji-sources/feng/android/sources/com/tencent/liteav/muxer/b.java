...
public class b implements a {
    ...
    private void c(ByteBuffer byteBuffer, BufferInfo bufferInfo) {
        ...
        if (j < this.q) {
            TXCLog.w("TXCMP4HWMuxer", "video is not in chronological order. current frame's pts(" + j + ") smaller than pre frame's pts(" + this.q + l.t);
        } else {
            this.q = j;
        }
        if (this.e != 2) {
    ...
    private void d(ByteBuffer byteBuffer, BufferInfo bufferInfo) {
        ...
        if (j < this.r) {
            TXCLog.e("TXCMP4HWMuxer", "audio is not in chronological order. current audio's pts pts(" + j + ") must larger than pre audio's pts(" + this.r + l.t);
            j = this.r + 1;
        } else {
            this.r = j;
        }
