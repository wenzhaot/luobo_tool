    ...
    public void run() {
        ...
        if (this.mFilePath == null || this.mFilePath.isEmpty()) {
            ...
            if (nativeStartPlay(this.mFilePath)) {
                ...
                try {
                    audioTrack = new AudioTrack(3, nativeGetSampleRate, i3, i2, AudioTrack.getMinBufferSize(nativeGetSampleRate, i3, i2), 1);
                    try {
                        audioTrack.play();
                        byte[] bArr = new byte[((nativeGetChannels * 2048) * 2)];
                        while (this.mIsRunning && !Thread.interrupted()) {
                            nativeGetSampleRate = nativeRead(bArr, bArr.length);
                            if (nativeGetSampleRate < 0) {
                                ...
                                audioTrack.write(bArr, 0, nativeGetSampleRate);
